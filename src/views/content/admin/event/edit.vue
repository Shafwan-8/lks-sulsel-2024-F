...
<template>
  ...
  const store = async () => {
    ...
    await Api.put(`/admin/event/${id.value}`, formData, {
      headers: {
  ...
  const getEvent = async () => {
    try 
    {
        const response = await Api.get(`/admin/event/${id.value}`, {
            headers: {
                Authorization: `Bearer ${token}`,
            },
        });
        ...
        date_start.value = response.data.data.date_start;
        date_end.value = response.data.data.date_end;
